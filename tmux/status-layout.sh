...
if [[ "$current" != "$state" ]]; then
  ...
  if (( state == 1 )); then
    ...
    tmux set-option -g status-format[1] '#[align=right]#(bash "${XDG_CONFIG_HOME:-$HOME/.config}/tmux/dw-status.sh" "#{pane_current_path}")#(bash "${XDG_CONFIG_HOME:-$HOME/.config}/tmux/git-status.sh" "#{pane_current_path}")#(bash "${XDG_CONFIG_HOME:-$HOME/.config}/tmux/vercel-status.sh" "#{pane_current_path}")#(bash "${XDG_CONFIG_HOME:-$HOME/.config}/tmux/codex-usage.sh" "#{pane_pid}" "#{window_id}")#(bash "${XDG_CONFIG_HOME:-$HOME/.config}/tmux/resource-status.sh")'
  else
    tmux set-option -g status-right '#(bash "${XDG_CONFIG_HOME:-$HOME/.config}/tmux/dw-status.sh" "#{pane_current_path}")#(bash "${XDG_CONFIG_HOME:-$HOME/.config}/tmux/git-status.sh" "#{pane_current_path}")#(bash "${XDG_CONFIG_HOME:-$HOME/.config}/tmux/vercel-status.sh" "#{pane_current_path}")#(bash "${XDG_CONFIG_HOME:-$HOME/.config}/tmux/codex-usage.sh" "#{pane_pid}" "#{window_id}")#(bash "${XDG_CONFIG_HOME:-$HOME/.config}/tmux/resource-status.sh")'
