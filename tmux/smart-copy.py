...
def detect(text: str) -> list[Target]:
    ...
    for row, line in enumerate(lines):
        ...
        if command and command.group(1).strip():
            add("command", command.group(1), row, display_column(line, command.start(1)))
    ...
    priority = {"location": 0, "url": 1, "path": 2, "error": 3, "command": 4,
                "codex-response": 5, "git-ref": 6, "git-hash": 7, "json": 8,
                "code": 9, "ip": 10, "timestamp": 11}
