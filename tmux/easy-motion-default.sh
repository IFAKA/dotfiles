...
if [[ -n "$raw_label" ]]; then
  ...
  if [[ "$mode" == "smart" && ! "${raw_label: -1}" =~ [A-Z] ]]; then
    # EasyMotion has already moved the native copy-mode cursor and started
    # the native selection. Smart Select only refines that selection.
