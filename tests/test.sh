...
grep -q 'fg=colour114,bg=colour235,bold]MEM ' <<<"$resource_status_raw" || fail "memory color or trailing spacing missing"
grep -q 'bg=colour235,fg=colour255,bold]#\[default\]$' <<<"$resource_status_raw" || fail "resource status trailing reset missing"
